<template>
  <div class="layout-col">
    ...
    <div class="layout-row overlay warp p8 ba1">
        <div class="p8 ww50" v-for="(v, i) in !isNull(publicStore._public.homeDevices)?
        ...
        state.active=='All'&&!state.airName?publicStore._public.homeDevices : [] : []" :key="i">
            <div class="ww100 flex-col ba13 rad6 i15 hidden">
              <div class="flex-col ww100 p15">
                ...
                <div class="flex1 flex-sc mt12">
                  <div class="flex3">SOC信息（%）</div>
                  <div class="flex1 tr">{{v.SOC?Math.floor((v.SOC*100))/100 : 0}}</div>
                </div>
                <div class="flex1 flex-sc mt12">
                  <div class="flex3">电量信息（kWh）</div>
                  <div class="flex1 tr">{{v.Electricity?Math.floor((v.Electricity*100))/100 : 0}}</div>
                </div>
                <div class="flex1 flex-sc mt12">
                  <div class="flex3">{{v.Operation_State=='0'?'充电功率':v.Operation_State=='1'?'发电功率':'实时功率'}}（kW）</div>
                  <div class="flex1 tr">{{v.Operation_State=='0'?Math.floor((v.Input_Power*100))/100:v.Operation_State=='1'?Math.floor((v.Output_Power*100))/100: 0}}</div>
                </div>
                <div class="flex1 flex-sc mt12">
                  <div class="flex3">{{v.Operation_State=='0'?'日充电量':v.Operation_State=='1'?'日发电量':'日充放电'}}（kWh）</div>
                  <div class="flex1 tr">{{v.Operation_State=='0'?Math.floor((v.Daily_Input*100))/100:v.Operation_State=='1'?Math.floor((v.Daily_Output*100))/100: 0}}</div>
                </div>
                <div class="flex1 flex-sc mt12">
                  <div class="flex3">额定容量（kW）</div>
                  <div class="flex1 tr">{{v.Total_Capacity?Math.floor((v.Total_Capacity*100))/100 : 0}}</div>
                </div>
              </div>
              <div class="flex-ec pt15 bgi22 ptb12">
                  <div class="mr20 flex-cc">
                      <div class="flex-sc btn-bg2 cursor ptb5 pl5 pr12" @click.stop="toPath('/detail', {id: v.id, stationNum: v.stationNum, type: v.type, code: 'Electricity', code2: 'SOC'})">
