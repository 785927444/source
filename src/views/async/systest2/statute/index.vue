...
<script lang="ts" setup>
  ...
  onMounted(async () => {
    await getInit();
    createScheduled("photovoltaic_control", 3 * 1000, () => {
      getDevicesData();
      getDevicesDailyOutput();
      getDevicesOutputPower();
    });
  });
  ...
  // 发电量
  const getDevicesDailyOutput = async() => {
    let name = 'DevicesDailyOutput'
  ...
  // 发电功率
  const getDevicesOutputPower = async() => {
    let name = 'DevicesOutputPower'
    let code = "Output_Power"
    let chart = [{name: '发电功率', code: code, data: []}]
    if (!proxy.isNull(publicStore._public.devices)){
      let query = getCodeQuery(code)
      let res = await publicStore.http({ Api: query })
      let list = proxy.isNull(res)? [] : res
