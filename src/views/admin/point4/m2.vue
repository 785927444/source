...
<script lang="ts" setup>
  ...
  const state = reactive({
    ...
    content: [],
    editFrom: [
      { required: true, editshow: true, name: '寄存器ID', key: 'address', type: 'input', regex: /^[1-9]\d*$/,msg: '寄存器ID必须为大于 0 的正整数'},
      ...
      { required: true, editshow: true,  name: '起始地址', key: 'point', type: 'input',regex: /^[0-9]+$/,msg: '起始地址必须为非负整数'  },
      { required: true, editshow: true,  name: '选择模式', key: 'mode', type: 'select', list: modes, value: 'value', label: 'name' },
      { required: true, editshow: true,  name: '生成数量', key: 'pointNum', type: 'input',regex: /^[1-9]\d*$/, msg: '生成数量必须为大于 0 的正整数' },
      { required: true, editshow: true,  name: '生成幅度', key: 'pointScale', type: 'input',regex: /^[1-9]\d*$/,msg: '生成幅度必须为大于 0 的正整数'  },
      { required: true, editshow: true,  name: '幅度系数', key: 'scale', type: 'input',regex: /^(0|([1-9]\d*))(\.\d+)?$/, msg: '幅度系数必须为非负数字（支持整数 / 小数）'},
      { required: true, editshow: true,  name: '偏移系数', key: 'offset', type: 'input',regex: /^(0|([1-9]\d*))(\.\d+)?$/, msg: '偏移系数必须为非负数字（支持整数 / 小数）'},
    ],
    export: [
    ...
    let content2 =  [
      { width: 'w70',   show: true, align: 'center', key: '*', name: '序号' },
      { width: 'w50x2', show: true, align: 'left', key: 'address', name: '寄存器ID', type: 'input', regex: /^[1-9]\d*$/, msg: '寄存器ID必须为大于 0 的正整数'  }, 
      ...
      { width: 'w50x2', show: true, align: 'left', key: 'len', name: '地址长度', type: 'input', regex: /^[1-9]\d*$/,msg: '地址长度必须为大于0的有效整数'  }, 
      { width: 'w50x2', show: true, align: 'left', key: 'point', name: '起始地址', type: 'input', regex: /^[0-9]+$/,msg: '起始地址不能为负数'  }, 
      { width: 'w50x2', show: true, align: 'left', key: 'mode', name: '模式', type: 'select', list: modes, value: 'value', label: 'name'  },
      { width: 'w50x2', show: true, align: 'left', key: 'scale', name: '幅度', type: 'input', regex: /^(0|([1-9]\d*))(\.\d+)?$/,msg: '幅度必须为有效数字（支持整数/小数）'},
      { width: 'w50x2', show: true, align: 'left', key: 'offset', name: '偏移', type: 'input', regex: /^(0|([1-9]\d*))(\.\d+)?$/,msg: '偏移必须为有效数字（支持整数/小数）' },
      { width: 'w50x5', show: true, align: 'left', key: 'code', name: '测量点', type: 'select', list: [], value: 'code', label: 'name'  },
  ...
  const handleChange = async(item) => {
    ...
    const validateItem = state.content.find(v => 
      typeof v.key === 'string' && v.key === fieldKey
    )
    ...
    if (validateItem) {
      ...
      // 2. 正则验证（格式+非负，正则已包含）
      if (!isEmpty && regex && typeof regex.test === 'function') {
