...
<script lang="ts" setup>
  ...
  const submitUpload = async() => {
    ...
    if (props.state.type == 't_v_104_point') {
     const deviceList = publicStore._public.tree || [];
     const validDeviceIds = deviceList.map(item => item.id + '');
     let invalidData = [];
     state.dataList.forEach(item => {
      const sensorId = item.sensorid ? item.sensorid + '' : '';
      if (!sensorId || !validDeviceIds.includes(sensorId)) {
        invalidData.push({
          ...item,
          errorMsg: `设备ID【${sensorId}】不存在，无法导入`
        });
      }
    });
    if (invalidData.length > 0) {
      state.isStatus = '2';
      state.isMsg = `检测到${invalidData.length}条数据的设备ID不存在，请检查后重新导入！`;
      state.errorList = invalidData;
      return;
    }
    }
