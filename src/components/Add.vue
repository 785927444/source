        ...
        <div v-for="(item, index) in editFromInfo" :key="index" :style="{width: props.state.editCol? (100/props.state.editCol) + '%' : 100 + '%' }" >
          <div v-if="!item.condit || dataForm[item.condit[0]]==item.condit[1]">
            <el-form-item :label="item.name" :prop="item.key" v-if="item.editshow != 'none' && (title == '创建' || item.editshow)">
              <!-- 输入框 -->
              <el-input size="large" v-model="dataForm[item.key]" placeholder="请输入" v-if="item.type=='input'" :type="item.class === 'positive-int' ? 'number' : (item.class?item.class:'text')"  :maxlength="item.maxlength?item.maxlength:''" :disabled="item.editshow=='disabledAll'?true:title=='修改'&&item.editshow=='disabled'?true:false"/>
              <!-- 密码框 -->
        ...
        if(item.required){
          if(item.editshow != 'none' && (title == '创建' || item.editshow)){
            let rule = proxy.varObj(item.key, [
              ...
              if(item.key === 'id'){
                rule.message = 'ID只能是数字';
                rule.pattern = /^[0-9]*$/;
              }else if(item.key === 'order'){
                rule.message = '排序只能输入正整数';
                rule.pattern = /^[1-9]\d*$/;
              }else{
                rule.message = item.type=='input'||item.type=='textarea'?'请输入':'请选择';
              }
          ...
          let orderRule = proxy.varObj('order', [
            {
              ...
              message: '排序只能输入正整数',
              trigger: 'blur',
              required: false // 非必填，有值时才校验
            }
          ])
          Object.assign(ruleList, orderRule)
        }
        if (item.key === 'instlPos' && item.type === 'input' && item.editshow != 'none' && (title == '创建' || item.editshow)) {
          ruleArr.push({
            max: 50,
            message: '安装位置长度不能超过50个字符',
            trigger: 'blur'
          });
        }
        if (ruleArr.length > 0) {
          let rule = proxy.varObj(item.key, ruleArr);
          Object.assign(ruleList, rule);
        }
        // 时间转化
        if(item.type=='time'){
  ...
  const handleSubmit = (formEl) => {
    formEl.validate (async valid => {
      if (valid) {
        ...
        const isDuplicate = props.state.list.some(item => {
          const itemId = Number(item.id);
          if (title === '修改') {
            const currentEditId = Number(dataForm[props.state.key]);
            return itemId === targetId && itemId !== currentEditId;
          }
      ...
      const inputName = dataForm.name;
      const inputSensorType = dataForm.sensor_type;
      if (inputName && inputSensorType) {
        const isNameDuplicate = props.state.list.some(item => {
          const isSameNameAndType = item.name === inputName && item.sensor_type === inputSensorType;
          if (title === '修改') {
            return isSameNameAndType && item[props.state.key] !== dataForm[props.state.key];
          }
          return isSameNameAndType;
        });
        
        if (isNameDuplicate) {
          ElNotification({ 
            title: '提示', 
            message: '该名称已存在，请输入其他名称', 
            type: 'error' 
          });
          return;
        }
